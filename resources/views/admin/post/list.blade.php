                    ...
                    <tbody>
                        ...
                        @foreach ($posts as $pr)
                            ...
                            <tr class="">
                                ...
                                <td>{{ $t }}</td>
                                <td><img src=" {{ asset($pr->thumbnail_post) }} " class="img-responsive"
                                        style="max-height: 100px; max-width:100px;" alt=""></td>
                                <td><a href="#">{{ substr($pr->post_title, 0, 30) }}...</a></td>

                                <td value="{{ $pr->catalog_post_id }}">{{ $pr->catalog_post_name }}</td>
                                <td>{{ $pr->created_at }}</td>
                                <td>
                                    <span
                                        class="badge {{ $pr->post_status == '1' ? 'badge-danger' : 'badge-success' }}">{{ $pr->post_status == '0' ? 'Công khai' : 'Chờ duyệt' }}</span>
                                </td>
                                <td>
                                    <a href="{{ route('edit.post', $pr->post_id) }}"
                                        class="btn btn-success btn-sm rounded-0 text-white" type="button"
                                        data-toggle="tooltip" data-placement="top" title="Edit"><i
                                            class="fa fa-edit"></i></a>
                                    <a href={{ route('delete.post', $pr->post_id) }}""
